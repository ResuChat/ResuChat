...
import { modifySection, replaceText, extractSectionContent } from '../src/lib/pdf/markdown'
import { parseResumeSections, sectionsToContentArray } from '../src/lib/pdf/pdfmaker'
...
describe('resume pdf heading rendering', () => {
  function hasDivider(content: Array<Record<string, unknown>>) {
    return content.some((item) => Array.isArray(item.canvas))
  }

  it('should not render divider under top-level h2 headings', () => {
    const sections = parseResumeSections('## 项目经验\n\n负责前端开发\n')
    const content = sectionsToContentArray(sections)

    expect(sections[0]?.level).toBe(2)
    expect(hasDivider(content)).toBe(false)
  })

  it('should render divider under h1 headings', () => {
    const sections = parseResumeSections('# 项目经验\n\n负责前端开发\n')
    const content = sectionsToContentArray(sections)

    expect(sections[0]?.level).toBe(1)
    expect(hasDivider(content)).toBe(true)
  })
})
