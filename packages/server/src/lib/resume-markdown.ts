...
export function modifySection(
  ...
  if (headingId && /^#[1-9]\d*$/.test(headingId)) {
    if (targetType === 'heading') {
      ...
      ;(heading as any).children = newTree.children
    } else if (spliceStart === spliceEnd) {
      const heading = children[targetIdx] as Heading
      ;(heading as any).children = newTree.children
    } else {
      const removeCount = spliceEnd - spliceStart
      ;(children as any[]).splice(spliceStart, removeCount, ...newTree.children)
    }
  } else {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (node.type === 'heading' && toString(node).trim() === fieldName) {
      ...
      for (let j = i + 1; j < children.length; j++) {
        ...
        parts.push(stringifier.stringify(child as any).trimEnd())
      }
      return parts.join('\n')
    }
  }
...
export function replaceText(fullText: string, current: string, newContent: string): string {
  ...
  if (headIdx === -1) {
    console.warn('[replaceText] current not found, returning original')
    console.warn('[replaceText] current:', current.slice(0, 80))
    return fullText
  }
  ...
  console.warn('[replaceText] all strategies failed, returning original')
  return fullText
}
